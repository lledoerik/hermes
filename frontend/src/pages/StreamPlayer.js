...
// HD icon for Real-Debrid player
const HDIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor">
  ...
  return (
    ...
    >
      ...
      <div className={`stream-controls-bar ${showControls ? 'visible' : ''}`}>
        ...
        {/* Botó HD - anar a DebridPlayer */}
        <button
          className="stream-btn stream-hd-btn"
          onClick={() => {
            const debridUrl = type === 'movie'
              ? `/debrid/movie/${tmdbId}`
              : `/debrid/tv/${tmdbId}?s=${season || 1}&e=${episode || 1}`;
            navigate(debridUrl);
          }}
          title="Qualitat HD (Real-Debrid)"
        >
          <HDIcon />
        </button>
